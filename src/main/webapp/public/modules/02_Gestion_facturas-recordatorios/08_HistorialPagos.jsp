        ...
        <div class="consultarFacturas__filtros">
            ...
            <div class="consultarFacturas__filtro-grupo">
                <a href="${pageContext.request.contextPath}/Facturas?accion=historialCategoria"
                   class="consultarFacturas__filtro-enlace ${param.accion == 'historialCategoria' ? 'consultarFacturas__filtro-enlace--activo' : ''}">
                   Categoría <img class="consultarFacturas__filtro-imagen" src="${pageContext.request.contextPath}/asset/imagenes/flecha.png" alt=""></a>
                <a href="${pageContext.request.contextPath}/Facturas?accion=historialFecha"
                   class="consultarFacturas__filtro-enlace ${param.accion == 'historialFecha' ? 'consultarFacturas__filtro-enlace--activo' : ''}">
                   Fecha <img class="consultarFacturas__filtro-imagen" src="${pageContext.request.contextPath}/asset/imagenes/flecha.png" alt=""></a>
                <a href="${pageContext.request.contextPath}/Facturas?accion=historialMonto"
                   class="consultarFacturas__filtro-enlace ${param.accion == 'historialMonto' ? 'consultarFacturas__filtro-enlace--activo' : ''}">
                   Monto <img class="consultarFacturas__filtro-imagen" src="${pageContext.request.contextPath}/asset/imagenes/flecha.png" alt=""></a>
            </div>
        </div>
                        ...
                        <div class="facturaCard facturaCard--verde">
                            ...
                            <div class="facturaCard__contenido">
                                <div class="facturaCard__encabezado">
                                    <%-- TÍTULO = categoría (igual que en consultar facturas) --%>
                                    <h3 class="facturaCard__titulo">${f.nombreCategoriaEgreso}</h3>
                                    <span class="facturaCard__etiqueta facturaCard__etiqueta--pagada">Pagada</span>
                                </div>
                                <c:choose>
                                    <c:when test="${not empty f.descripcionPago}">
                                        <p class="facturaCard__detalles">${f.descripcionPago}</p>
                                    </c:when>
                                    <c:otherwise>
                                        <p class="facturaCard__detalles" style="color:#aaa;font-style:italic;">Sin descripción</p>
                                    </c:otherwise>
                                </c:choose>
                                <p class="facturaCard__detalles">Monto: <fmt:formatNumber value="${f.monto}" type="currency" currencySymbol="$"/></p>
                                <p class="facturaCard__detalles">Pagado el: ${f.fechaPagoFormateada}</p>
                            </div>
                        </div>
                    </c:forEach>
                </c:otherwise>
